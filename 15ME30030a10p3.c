...
void mult_poly (PolyTerm *poly1_ptr,int max_exp_poly_1,PolyTerm *poly2_ptr,int max_exp_poly_2,PolyTerm *polyresult_ptr)
{
  int max_exp=max_exp_poly_1+max_exp_poly_2;
  int i,j;
  for(i=0;i<=max_exp;i++)
    {
      (polyresult_ptr+i)->coefficient=0;
      (polyresult_ptr+i)->exponent=max_exp-i;
    }
  for(i=0;i<=max_exp_poly_1;i++)
    for(j=0;j<=max_exp_poly_2;j++)
      (polyresult_ptr+i+j)->coefficient+=((poly1_ptr+i)->coefficient)*((poly2_ptr+j)->coefficient);
}
